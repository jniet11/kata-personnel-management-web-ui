...
export default function EditAccessRequestPage() {
  ...
  const [requestData, setRequestData] = useState<AccessRequestEditData | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    if (requestId) {
      ...
      const fetchRequestDetails = axios.get<{
        success: boolean;
        data?: AccessRequestEditData;
        error?: string;
      }>(
        `http://localhost:4000/personnel-management/get-access-request-by-id/${requestId}`,
        authHeaders
      );
      ...
      Promise.all([fetchRequestDetails, fetchApprovedUsersList])
        .then(([requestDetailsResponse, approvedUsersResponse]) => {
          if (requestDetailsResponse.status === 401) {
            alert("Sesión expirada o no autorizado. Redirigiendo al login.");
            router.push("/login");
            return;
          }
          if (
            requestDetailsResponse.data.success &&
            ...
            setEditedAccessTypes(
              fetchedData.access_type
                ? fetchedData.access_type.split(",").map((item) => item.trim())
                : []
            );
          } else {
            setError(
              requestDetailsResponse.data.error ||
                "No se pudo cargar la solicitud."
            );
          }

          if (approvedUsersResponse.status === 401) {
            alert("Sesión expirada o no autorizado. Redirigiendo al login.");
            router.push("/login");
            return;
          }
        ...
        .catch((err) => {
          ...
          if (axios.isAxiosError(err) && err.response?.status === 401) {
            ...
            if (err.config?.url?.includes("get-access-request-by-id")) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axios.put(
        `http://localhost:4000/personnel-management/update-access-request/${requestId}`,
        {
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        alert(
          "Tu sesión ha expirado o no tienes permiso. Redirigiendo al login."
        );
        router.push("/login");
      } else if (axios.isAxiosError(err) && err.response) {
        setError(
          err.response.data?.error ||
            "Error al actualizar la solicitud de acceso."
        );
      } else {
  ...
  if (error && !requestData && !isFetchingData)
    return (
      <div className="container mx-auto p-6 text-center text-red-500">
        {error}
      </div>
    );
  if (!requestData && !isFetchingData)
    return (
